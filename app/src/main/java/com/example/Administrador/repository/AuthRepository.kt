...
import com.example.Administrador.model.login.SendCodeRequestDTO
import com.example.Administrador.model.login.UsuarioResponseDTO
import com.example.Administrador.model.login.VerifyCodeRequestDTO
...
class AuthRepository {
    ...
    private fun extractErrorMessage(errorBody: String?): String {
        if (errorBody.isNullOrBlank()) return "Error inesperado del servidor"

        return try {
            val gson = com.google.gson.Gson()
            val apiError = gson.fromJson(errorBody, com.example.Administrador.model.inventario.ApiErrorDTO::class.java)
            val message = apiError.message ?: ""
            
            // Si el backend envía un error 500 pero el mensaje indica duplicado
            if (message.contains("ya tiene un usuario asignado", ignoreCase = true) || 
                message.contains("ya está registrado", ignoreCase = true)) {
                "Este correo ya está registrado. Por favor, usa otro o inicia sesión."
            } else {
                message.ifBlank { "Error del servidor: $errorBody" }
            }
        } catch (e: Exception) {
            try {
                val json = org.json.JSONObject(errorBody)
                val msg = json.optString("message", errorBody)
                if (msg.contains("ya tiene un usuario asignado", ignoreCase = true)) {
                    "Este correo ya está registrado. Por favor, usa otro o inicia sesión."
                } else {
                    msg
                }
